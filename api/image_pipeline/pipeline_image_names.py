...
# Driver functions
def pipeline_from_github(github_repo: str, version: str) -> Tuple[dict, str]:
    """
    ...
    if not pipeline_image_util.github_repo_is_available(github_repo):  # Check if the given GitHub repo actually exists
        ...
        for distgit_repo_name in distgit_repos:
            ...
            github_object.distgit += [distgit_object]
...
def pipeline_from_distgit(distgit_repo_name, version) -> Tuple[dict, str]:
    """
...
def pipeline_from_brew(brew_name, version) -> Tuple[dict, str]:
    """
...
def pipeline_from_cdn(cdn_repo_name, version) -> Tuple[dict, str]:
    """
...
def pipeline_from_delivery(delivery_repo_name, version) -> Tuple[dict, str]:
    """
